...
def hello(message):
    ...
    bot.send_message(message.chat.id, 'Привет, {0.first_name}✋! Что хотите узнать?'.format(message.from_user, bot.get_me()), reply_markup=markup) #Поздороваться обязательно
...
def callback_inline(call): #Для маленькой клавиатуры
    try:
        if call.message:
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,text='Раписание звонков',
